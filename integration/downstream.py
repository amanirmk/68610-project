...
NLVR_TRAIN_LIMIT = 5_000  # x6 examples, use 5k
VQA2_TRAIN_LIMIT = 30_000  # use 30k
VQA2_EVAL_LIMIT = 30_000  # use 30k
...
def evaluate_downstream(args):
        ...
        for model_name in args.model_names:
            try:
                exact_acc, partial_acc = finetune_vqa2(
                    model_name,
                    save_intermediate=args.save_intermediate,
                    train_limit=VQA2_TRAIN_LIMIT,
                    eval_limit=VQA2_EVAL_LIMIT,
                )
            except Exception as e:  # pylint: disable=broad-exception-caught
                DownstreamEval.error(f"Failed to evaluate {model_name}: {e}")
                DownstreamEval.error(traceback.format_exc())
    ...
    if args.do_nlvr_zeroshot:
        ...
        for model_name in args.model_names:
            try:
                precision, consistency = zeroshot_nlvr(
                    model_name, save_intermediate=args.save_intermediate
                )
            except Exception as e:  # pylint: disable=broad-exception-caught
                DownstreamEval.error(f"Failed to evaluate {model_name}: {e}")
                DownstreamEval.error(traceback.format_exc())
    ...
    if args.do_nlvr_finetune:
        ...
        for model_name in args.model_names:
            try:
                precision, consistency = finetune_nlvr(
                    model_name,
                    save_intermediate=args.save_intermediate,
                    train_limit=NLVR_TRAIN_LIMIT,
                )
            except Exception as e:  # pylint: disable=broad-exception-caught
                DownstreamEval.error(f"Failed to evaluate {model_name}: {e}")
                DownstreamEval.error(traceback.format_exc())
