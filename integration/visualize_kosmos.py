...
def main(argv):
    ...
    model, processor = load_model_and_processor("microsoft/kosmos-2-patch14-224", padding_side='right')

    image = Image.open(image_path)
    inputs = processor(text="<grounding> " + sentence, images=image, return_tensors='pt').to('cuda', torch.float16)

    processed_image_raw = inputs['pixel_values'][0].permute(1, 2, 0).cpu().numpy()
    processed_image_raw = (processed_image_raw - processed_image_raw.min()) / (processed_image_raw.max() - processed_image_raw.min()) * 255
    ...
    for i, token in enumerate(tokens):
        token_results.append(
            (
              token,
              attentions[:, i].cpu().numpy().reshape(8, 8)
            )
        )
    ...
    for token, attention in token_results:
        if token == '<grounding>':
            ...
            i += 1
            normed_attention = (attention - attention.min()) / (attention.max() - attention.min()) * 255
